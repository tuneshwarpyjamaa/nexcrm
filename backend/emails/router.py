...
def _row_to_email(row) -> dict:
    return {
        ...
        "subject": row["subject"],
        "body": row["body"],
        "trackingId": row.get("trackingId") or row.get("trackingid"),
        "openCount": row.get("openCount") or row.get("opencount") or 0,
        "lastOpenedAt": str(row.get("lastOpenedAt") or row.get("lastopenedat")) if (row.get("lastOpenedAt") or row.get("lastopenedat")) else None,
        "isRead": bool(row.get("isRead") or row.get("isread")),
        "readAt": str(row.get("readAt") or row.get("readat")) if (row.get("readAt") or row.get("readat")) else None,
        "direction": row.get("direction") or "sent",
        "contactId": row.get("contactId") or row.get("contactid"),
        "type": row.get("type"),
        "sentAt": str(row.get("sentAt") or row.get("sentat") or ""),
    }
